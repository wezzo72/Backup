#!/usr/bin/env node
// Fast parallel V5 re-stamp — pre-loads the 4 portfolio PNGs once,
// then processes all PDFs 10 at a time using Promise.all concurrency.
'use strict';
const { PDFDocument, StandardFonts, rgb } = require('pdf-lib');
const fs   = require('fs');
const path = require('path');

const STAMP_MARKER  = 'BDLETF-STAMPED-V5';
...
function addLink(page, pdfDoc, url, x, y, w, h) {
  try {
    const { PDFName, PDFHexString, PDFDict, PDFArray, PDFNumber } = require('pdf-lib');
...
async function stampWithPortfolio(filePath, pngBytesArr) {
  ...
  try {
    ...
    // Always force-stamp so portfolio pages are guaranteed to be present
    ...
    // Pre-embed all 4 JPEGs for this document
    const pngImages = await Promise.all(pngBytesArr.map(b => pdfDoc.embedJpg(b)));

    // Add 4 portfolio pages
    for (let i = 0; i < COVERS.length; i++) {
      ...
      page.drawText('CREATIVE PORTFOLIO  ·  DR. RICHARD McLEAN', { x: LM, y: H - 22, size: 8,  font: rf, color: WHITE });
      page.drawText(c.title, { x: LM, y: H - 44, size: 17, font: bf, color: WHITE });
      page.drawText(c.sub,   { x: LM, y: H - 62, size: 9,  font: rf, color: WHITE });
      page.drawText(`${c.pages} pages  \xB7  Free to view at simplebooklet.com  \xB7  No login required`, { x: LM, y: H - 76, size: 7, font: mf, color: WHITE });
      ...
      page.drawText('Click to view this portfolio online:', { x: LM, y: 44, size: 9,  font: rf, color: NAVY });
      page.drawText(c.url,                                   { x: LM, y: 27, size: 10, font: bf, color: NAVY });
      page.drawText('Free  \xB7  No login required  \xB7  simplebooklet.com', { x: LM, y: 11, size: 7, font: mf, color: NAVY });
...
async function main() {
  // Pre-load PNGs once
  console.log('Loading portfolio screenshots…');
  const pngBytesArr = COVERS.map(c => {
    const p = path.join(SCREENSHOT_DIR, c.file);
    if (!fs.existsSync(p)) throw new Error(`Missing screenshot: ${c.file}`);
    return fs.readFileSync(p);
  });
  console.log('  Loaded 4 PNGs.\n');

  const all = getAllPDFs(DOCS_DIR);
  console.log(`Found ${all.length} PDFs. Stamping (10 parallel)…\n`);
  ...
  async function worker() {
    while (idx < all.length) {
      ...
      const res  = await stampWithPortfolio(file, pngBytesArr);
      const n    = i + 1;
  ...
  console.log(`\n✓ Done: ${stamped} stamped · ${already} already up-to-date · ${skipped} skipped · ${failed} failed`);
}
